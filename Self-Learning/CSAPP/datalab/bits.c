...
int isTmax(int x) {
  ...
  // max + 1 = 100000...
  // ~(max+1) = 011111... = max 
  // 111111... + 1 = 000000.... , ~0x0 = 1111111 ,this situation needs to be ignored 
  return !(~(x+1) ^ x) & !!((x+1) ^ 0x0);
  // Use XOR(^) to determine 'equality'
}
/* 
 ...
 */
int negate(int x) {
  // return x + (1 << 31) 这里错误的原因是，位操作对象都是补码而不是真值。
  return ~x + 1;
}
//3
/* 
...
int howManyBits(int x) {
  ...
  if x < 0 return */
  int flag, bit_16, bit_8, bit_4, bit_2, bit_1, bit_0;
  flag = x >> 31; // < 0 : 11111111; > 0 : 00000000 
  x = ((~flag) & x) | (flag & (~x));
